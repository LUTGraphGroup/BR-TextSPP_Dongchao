import csv
import pickle, random, torch, logging,  re, gc
import numpy as np
import pandas as pd
...
class MIMIC:
    def __init__(self, ADMISSIONSPATH="ADMISSIONS.csv",
...
class MIMIC_new:
    def __init__(self, ADMISSIONSPATH="ADMISSIONS.csv", NOTEEVENTSPATH="NOTEEVENTS.csv",
                 ...
                 PROCEDURESICDPATH="PROCEDURES_ICD.csv",
                 DICDPROCEDURESPATH="D_ICD_PROCEDURES.csv"):
        self.ADMISSIONSPATH = ADMISSIONSPATH
        self.NOTEEVENTSPATH = NOTEEVENTSPATH
        self.DIAGNOSESICDPATH = DIAGNOSESICDPATH
        self.DICDDIAGNOSESPATH = DICDDIAGNOSESPATH
...
class DataClass:
    def __init__(self, dataPath="data.csv", mimicPath='mimic3/caml/MIMIC3', stopWordPath="stopwords.txt",
                 validSize=0.2, testSize=0.0, minCount=10, noteMaxLen=768, seed=9527, topICD=-1):
        term_pattern = re.compile('[A-Za-z0-9]+|[,;.!?()]|<br>|<:>', re.I)
        self.minCount = minCount
        validSize *= 1.0 / (1.0 - testSize)
        print('Begin processing raw data......')
        data = pd.read_csv(dataPath, usecols=['HADM_ID', 'TEXT', 'ICD9_CODE'])
        print('Perform analysis operations on clinical records and ICD codes......')
        ...
        print('Discard some insignificant words......')
        ...
        self.twordNum = cnt
        print('Obtain the tokens of clinical records and ICD titles......')
    ...
    def vectorize(self, method="skipgram", noteFeaSize=320, titleFeaSize=192, window=5, iters=10, batchWords=1000,
                  noteCorpusPath="note_corpus.txt", workers=8, loadCache=True, suf=""):
        ...
        if os.path.exists(path) and loadCache:
            ...
            if method == 'skipgram':
                model = Word2Vec(corpus, min_count=self.minCount, window=window, vector_size=noteFeaSize,
                                 workers=workers,sg=1, epochs=iters, batch_words=batchWords)
                ...
                model = Word2Vec(corpus, min_count=self.minCount, window=window, size=noteFeaSize, workers=workers,
                                 sg=0, iter=iters, batch_words=batchWords)
                word2vec = np.zeros((self.nwordNum, noteFeaSize), dtype=np.float32)
                for i in range(self.nwordNum):
                    word2vec[i] = model.wv[self.id2nword[i]] if self.id2nword[i] in model.wv else np.random.random(
                        noteFeaSize)
                self.vector['noteEmbedding'] = word2vec
            elif method == 'MLM':
        ...
        if os.path.exists(path) and loadCache:
            ...
            if method == 'skipgram':
                model = Word2Vec(self.titles, min_count=0, window=window, vector_size=titleFeaSize, workers=workers, sg=1,
                                 epochs=1)    # iter=10
                ...
                model = Word2Vec(self.titles, min_count=0, window=window, vector_size=titleFeaSize, workers=workers, sg=0,
                                 epochs=10)
                word2vec = np.zeros((self.twordNum, titleFeaSize), dtype=np.float32)
                for i in range(self.twordNum):
                    word2vec[i] = model.wv[self.id2tword[i]]
...
def get_ICD_vectors(dataClass, mimic3path="mimic3/caml/3"):
    dia_icd = pd.read_csv(os.path.join(mimic3path, 'D_ICMIMICD_DIAGNOSES.csv'))
