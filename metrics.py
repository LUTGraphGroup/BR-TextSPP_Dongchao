...
os.environ['CUDA_LAUNCH_BLOCKING'] = '1'   # Run with a specific graphics card specified
...
def AUC(classNum, Y_prob_pre, Y, average='micro'):
    assert average in ['micro', 'macro'], "average 参数必须是 'micro' 或 'macro' 之一"
    if len(np.unique(Y)) < 2:
        raise ValueError("Only one class present in y_true. ROC AUC score is not defined in that case.")
    if classNum == 2:
        auc = roc_auc_score(Y, Y_prob_pre[:, 1])
    else:
        auc = roc_auc_score(Y, Y_prob_pre, average=average, multi_class='ovr')
    return auc
